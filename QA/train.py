...
def train(model, train_loader, eval_loader, num_epochs, device, batch_size, grad_acc_steps = 1, eval_batch_size=2048):
    criterion = nn.BCEWithLogitsLoss(reduction="mean")
    optimizer = AdamW(model.parameters(), lr=4e-5, eps=1e-6, weight_decay= 10e-4, correct_bias=False) 
    ...
    iter_step = 1
    model.train()
    wandb.watch(model, criterion, log_freq=1000)
    for epoch in range(num_epochs):
        ...
        for i, (text, image, target) in enumerate(train_loader):
            ...
            batch_score = compute_score_with_logits(pred, target).sum() / float(batch_size) # average
        ...
        model.eval()
        eval_score, eval_loss = evaluate(model, eval_loader, device, criterion, eval_batch_size)
    ...
    PATH = "/home/ndp689/iglue_gqa/iglue_GQAXMLCLIP.pt"
    # save model
    torch.save(model.state_dict(), PATH)
    wandb.finish()

def evaluate(model, val_loader, device, criterion, batch_size = 2048):
    eval_score = 0
    eval_loss = 0
    num_data = 0
    with torch.no_grad():
        for text, image, target in iter(val_loader):
            ...
            batch_score = compute_score_with_logits(pred, target).sum() / float(batch_size)
