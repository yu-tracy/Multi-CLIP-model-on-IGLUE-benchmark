...
def train(model, train_loader, eval_loader, num_epochs, device, batch_size, grad_acc_steps = 1):
    criterion = nn.CrossEntropyLoss()
    optimizer = AdamW(model.parameters(), lr=2e-5, eps=1e-6, weight_decay= 10e-4, correct_bias=False) 
    ...
    for epoch in range(num_epochs):
        ...
        for i, (image, text, target, image_id) in enumerate(train_loader):
            
            num_options = text.size(1)
            text, target, image = text.to(device), target.to(device), image.to(device)
            pred = model(image, text)

            vil_logit = pred.view(text.shape[0], num_options)
            loss = criterion(vil_logit, target)

            _, preds = torch.max(vil_logit, 1)
            batch_score = float((preds == target).sum()) / float(text.shape[0])
        ...
        model.eval()
        eval_score, eval_loss = evaluate(model, eval_loader, criterion, device)
...
def evaluate(model, val_loader, criterion, device):
    ...
    eval_loss = 0
    with torch.no_grad():
        for image, text, target, image_id in iter(val_loader):
            num_options = text.size(1)
            text, target, image = text.to(device), target.to(device), image.to(device)
            pred = model(image, text)

            vil_logit = pred.view(text.shape[0], num_options)
            batch_loss = criterion(vil_logit, target)
            _, preds = torch.max(vil_logit, 1)
            batch_score = float((preds == target).sum()) / float(text.shape[0])
